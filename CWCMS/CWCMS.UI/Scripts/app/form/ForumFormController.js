app.controller("forumFormController", ["$scope", "$http", "$location", '$timeout', '$window', function ($scope, $http, $location, $timeout, $window) {
    ...
    $scope.countAnnouncements = function () {
        if ($scope.listingMode == 1) {
            $http.get('api/forum/getcount').success(function (data) {
        ...
        .error(function () {
            ...
            $http.get('api/forum/HeaderGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
		.error(function () {
            ...
            $http.get('api/forum/ContentGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
        .error(function () {
    ...
    $scope.getJsonFromUrl = function () {
        ...
        $http.get('api/forum/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.CircularList = function () {
        $http.get('api/forum/Outlist').success(function (data) {
            $scope.Circular = data;
            $scope.loading = false;
        }).error(function () {
            $scope.error = "An Error has occured while loading posts!"
    ...
    $scope.signList = function () {
        $http.get('api/forum/takeSign').success(function (data) {
            $scope.signList = data;
            $scope.SignList = data;
            $scope.loading = false;
    ...
    $scope.departmentList = function () {
        $http.get('api/forum/takeDepartment').success(function (data) {
    ...
    $scope.refreshDepartment = function () {
        $http.get('api/forum/takeDepartment')
              .success(function (data) {
                  $scope.Department = data;
              });
    }
    ...
    $scope.listrefresh = function () {
        ...
        if (typeof pageLinkSize[2] == 'undefined' || typeof pageLinkSize[4] == 'undefined' || typeof pageLinkSize[6] == 'undefined') {
            ...
            if (pageLinkSize[6] == '1') {
                ...
                $scope.forums = []; $http.get('api/forum/searchheader/' + $scope.query1).success(function (data) {
                    $scope.forums = data;
                ...
                $scope.forums = []; $http.get('api/forum/searchcontent/' + $scope.query2).success(function (data) {
                    $scope.forums = data;
                    $scope.total_count = data.total_count;
    ...
    $scope.addtothelink = function (ForumId) {

        $http.get('api/forum/getforumname/' + ForumId).success(function (data) {
            $scope.linkedForumName = data.ForumName;
            $scope.linkedForumId = data.ForumId;
            $scope.linkedForumText = data.Text;
    ...
    $scope.relateTypes = [
        { types: 'Formun Kapsamı Genişletildi' },
        { types: 'Form Geçerliliğini Yitirdi' },
	...
	$scope.clear = function () {
        ...
        $scope.Forum.ForumName = '';
        $scope.Forum.Text = '';
        $scope.Forum.FilePath = '';
        $scope.Forum.DepartmentCode = '';
    }
    $scope.refresh = function () {
        $http.get('api/forum/search')
              .success(function (data) {
                  $scope.Forum = data;
              });
    }
	...
	$scope.dateFix = function () {
        var date = $scope.Forum.DateExpireTime;
	...
	$scope.save = function () {
        {
            if ($scope.Forum.ForumName == "" || $scope.Forum.Text == "") {
                alert("Lütfen Form Bölümünü Boş Bırakmayınız.");
            } else {
                $scope.Forum.DepartmentName = $scope.departmentList;
                $scope.Forum.Signatory = $scope.signList;
                $http({
                    method: 'POST',
                    url: 'api/forum/save',
                    data: $scope.Forum
					...
					$scope.Link = {
                        ...
                        LinkId: '',
                        LinkedId: $scope.linkedForumId,
                        LinkerId: $scope.forumData.ForumId,
                        LinkedCategoryCode: 'F',
                        LinkerCategoryCode: 'F',
                        LinkPath: $scope.Forum.FilePath,
                        LinkedSituation: $scope.selectedRelation,
                        LinkerAnnouncementName: $scope.Forum.ForumName
                    }

                    //////////////
                    $scope.clear();
                    alert(response.data.ForumId + " No'lu Form " + "Başarıyla Eklendi !");
                    if ($scope.linkedForumId > 0) {
                        $scope.linksave();
                    }
                    location.reload();
    ...
    $scope.diffcompare = function () {
        ...
        var text1 = $scope.linkedForumText;
        var text2 = $scope.Forum.Text;
    ...
    $scope.getLinkedText = function (id1) {
        $http.get('api/forum/getforumtext/' + id1).success(function (data) {
            $scope.comparedtext = "";
	...
	$scope.getLinkerText = function (id2) {
        $http.get('api/forum/getforumtext/' + id2).success(function (data) {
    ...
    $scope.diffcompareview = function (combination) {
        ...
        var idArr = str.split("~");
        if (idArr[1] == null) {
            alert("Form Bağlı Değil İçerik Karşılaştırılamadı !");
        }
        else {
    ...
    $scope.remove = function (ForumId) {
        $scope.loading = true;
        //var Id = this.Announcement.AnnouncementId;
        $http({
            method: 'PUT',
            url: 'api/forum/remove/' + ForumId,
            data: { data: ForumId }
        }).then(function successCallback(response) {
            //$scope.announcementData.push(response.data);
    ...
    $scope.uploadFiles = function () {
        ...
        $http(request)
            .success(function (d) {
                ...
                $scope.Forum.FilePath = d.FileName;
    ...
    $scope.downloadFile = function (name, ForumId) {
        $http({
            method: 'GET',
            url: 'api/forum/download' + ForumId,
            params: {
                name: name,
                AnnId: AnnId
            },
