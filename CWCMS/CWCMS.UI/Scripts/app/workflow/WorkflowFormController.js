app.controller("workflowFormController", ["$scope", "$http", "$location", '$timeout', '$window', function ($scope, $http, $location, $timeout, $window) {
    ...
    $scope.modalclear = function () {
        ...
        $scope.Workflow.Text = '';
        $scope.Workflow.DepartmentCode= '';
        $scope.Workflow.SignId='';
        $scope.linkedWorkflow = '';
        $scope.matchedtext ='';
    ...
    $scope.countAnnouncements = function () {
        if ($scope.listingMode == 1) {
            $http.get('api/workflow/getcount').success(function (data) {
        ...
        .error(function () {
            ...
            $http.get('api/workflow/HeaderGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
		.error(function () {
            ...
            $http.get('api/workflow/ContentGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
        .error(function () {
    ...
    $scope.getJsonFromUrl = function () {
        ...
        $http.get('api/workflow/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.allworkflowdata = function () {
        $scope.stringnumcomb = $scope.firstIndex + "-" + $scope.itemsperpage;
        $http.get('api/workflow/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.CircularList = function () {
        $http.get('api/workflow/Outlist').success(function (data) {
            $scope.Circular = data;
            $scope.loading = false;
        }).error(function () {
            $scope.error = "An Error has occured while loading posts!"
    ...
    $scope.signList = function () {
        $http.get('api/workflow/takeSign').success(function (data) {
            $scope.signList = data;
            $scope.SignList = data;
            $scope.loading = false;
    ...
    $scope.refreshSignList = function () {
        $http.get('api/workflow/takeSign')
    ...
    $scope.departmentList = function () {
        $http.get('api/workflow/takeDepartment').success(function (data) {
    ...
    $scope.refreshDepartment = function () {
        $http.get('api/workflow/takeDepartment')
              .success(function (data) {
                  $scope.Department = data;
              });
    }
    ...
    $scope.listrefresh = function () {
        ...
        if (typeof pageLinkSize[2] == 'undefined' || typeof pageLinkSize[4] == 'undefined' || typeof pageLinkSize[6] == 'undefined') {
            ...
            if (pageLinkSize[6] == '1') {
                ...
                $scope.workflows = []; $http.get('api/workflow/searchheader/' + $scope.query1).success(function (data) {
                    $scope.workflows = data;
                ...
                $scope.workflows = []; $http.get('api/workflow/searchcontent/' + $scope.query2).success(function (data) {
                    $scope.workflows = data;
                    $scope.total_count = data.total_count;
    ...
    $scope.addtothelink = function (WorkflowId) {

        $http.get('api/workflow/getworkflowname/' + WorkflowId).success(function (data) {
            $scope.linkedWorkflowName = data.WorkflowName;
            $scope.linkedWorkflowId = data.WorkflowId;
            $scope.linkedWorkflowText = data.Text;
	...
	// Viewing Circular
    $scope.viewWorkflow = function (WorkflowId) {
        $http.get('api/workflow/getworkflowname/' + WorkflowId).success(function (data) {
            $scope.viewedWorkflow = data;
            $scope.loading = false;
        })
    }
    ///
    ...
    $scope.relateTypes = [
        { types: 'İş Akışının Kapsamı Genişletildi' },
        { types: 'İş Akışı Geçerliliğini Yitirdi' },
	...
	$scope.clear = function () {
        ...
        $scope.Workflow.WorkflowName = '';
        $scope.Workflow.Text = '';
        $scope.Workflow.FilePath = '';
        $scope.Workflow.DepartmentCode = '';
    }
    $scope.refresh = function () {
        $http.get('api/workflow/search')
              .success(function (data) {
                  $scope.Workflow = data;
              });
    }
	...
	$scope.dateFix = function () {
        var date = $scope.Workflow.DateExpireTime;
	...
	$scope.save = function () {
        {
            if ($scope.Workflow.WorkflowName == "" || $scope.Workflow.Text == "") {
                alert("Lütfen İş Akışı Bölümünü Boş Bırakmayınız.");
            } else {
                $scope.Workflow.DepartmentName = $scope.departmentList;
                $scope.Workflow.Signatory = $scope.signList;
                $http({
                    method: 'POST',
					...
					$scope.Link = {
                        ...
                        LinkId: '',
                        LinkedId: $scope.linkedWorkflowId,
                        LinkerId: $scope.workflowData.WorkflowId,
                        LinkedCategoryCode: 'A',
                        LinkerCategoryCode: 'A',
                        LinkPath: $scope.Workflow.FilePath,
                        LinkedSituation: $scope.selectedRelation,
                        LinkerAnnouncementName: $scope.Workflow.WorkflowName
                    }

                    //////////////
                    $scope.clear();
                    alert(response.data.WorkflowId + " No'lu İş Akışı " + "Başarıyla Eklendi !");
                    if ($scope.linkedWorkflowId > 0) {
                        $scope.linksave();
                    }
                    location.reload();
    ...
    $scope.diffcompare = function () {
        ...
        var text1 = $scope.linkedWorkflowText;
        var text2 = $scope.Workflow.Text;

        $http({
            method: 'POST',
            url: 'api/workflow/diff',
    ...
    $scope.getLinkedText = function (id1) {
        $http.get('api/workflow/getworkflowtext/' + id1).success(function (data) {
            $scope.comparedtext = "";
	...
	$scope.getLinkerText = function (id2) {
        $http.get('api/workflow/getworkflowtext/' + id2).success(function (data) {
    ...
    $scope.diffcompareview = function (combination) {
        ...
        var idArr = str.split("~");
        if (idArr[1] == null) {
            alert("İş Akışı Bağlı Değil İçerik Karşılaştırılamadı !");
        }
        else {
    ...
    $scope.remove = function (WorkflowId) {
        $scope.loading = true;
        //var Id = this.Announcement.AnnouncementId;
        $http({
            method: 'PUT',
            url: 'api/workflow/remove/' + WorkflowId,
    ...
    $scope.uploadFiles = function () {
        ...
        $http(request)
            .success(function (d) {
                ...
                $scope.Workflow.FilePath = d.FileName;
    ...
    $scope.downloadFile = function (name, WorkflowId) {
        $http({
            method: 'GET',
            url: 'api/workflow/download' + WorkflowId,
            params: {
                name: name,
                AnnId: AnnId
            },
