app.controller("legislationFormController", ["$scope", "$http", "$location", '$timeout', '$window', function ($scope, $http, $location, $timeout, $window) {

	$scope.legislationData = '';
	$scope.Legislation = {
        LegislationId: '',
        LegislationName: '',
		Text: '',
    ...
    $scope.modalclear = function () {
        ...
        $scope.Legislation.Text = '';
        $scope.Legislation.DepartmentCode= '';
        $scope.Legislation.SignId='';
    ...
    $scope.diffanycompareclear = function () {
        $scope.comparedtext = "";
        $scope.legislation.LinkerId = "";
        $scope.legislation.LegislationId = "";
    ...
    $scope.getJsonFromUrl = function () {
        ...
        $http.get('api/legislation/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.alllegislationdata = function () {
        $scope.stringnumcomb = $scope.firstIndex + "-" + $scope.itemsperpage;
        $http.get('api/legislation/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.listrefresh = function () {
        ...
        if (typeof pageLinkSize[2] == 'undefined' || typeof pageLinkSize[4] == 'undefined' || typeof pageLinkSize[6] == 'undefined') {
            ...
            if (pageLinkSize[6] == '1') {
                ...
                $scope.legislations = []; $http.get('api/legislation/searchheader/' + $scope.query1).success(function (data) {
                    $scope.legislations = data;
                ...
                $scope.legislations = []; $http.get('api/legislation/searchcontent/' + $scope.query2).success(function (data) {
                    $scope.legislations = data;
                    $scope.total_count = data.total_count;
    ...
    $scope.addtothelink = function (LegislationId) {

        $http.get('api/legislation/getlegislationname/' + LegislationId).success(function (data) {
            $scope.linkedLegislationName = data.LegislationName;
            $scope.linkedLegislationId = data.LegislationId;
            $scope.linkedLegislationText = data.Text;
	...
	// Viewing Circular
    $scope.viewLegislation = function (LegislationId) {
        $http.get('api/legislation/getlegislationname/' + LegislationId).success(function (data) {
            $scope.viewedLegislation = data;
            $scope.loading = false;
        })
    }
    ///
    ...
    $scope.relateTypes = [
        { types: 'Yönetmeliğin Kapsamı Genişletildi' },
        { types: 'Yönetmelik Geçerliliğini Yitirdi' },
	...
	$scope.clear = function () {
        ...
        $scope.Legislation.LegislationName = '';
        $scope.Legislation.Text = '';
        $scope.Legislation.FilePath = '';
        $scope.Legislation.DepartmentCode = '';
    }
    $scope.refresh = function () {
        $http.get('api/legislation/search')
              .success(function (data) {
                  $scope.Legislation = data;
              });
    }
	
	$scope.dateFix = function () {
        var date = $scope.Legislation.DateExpireTime;
	...
	$scope.save = function () {
        {
            if ($scope.Legislation.LegislationName == "" || $scope.Legislation.Text == "") {
                alert("Lütfen Yönetmelik Bölümünü Boş Bırakmayınız.");
            } else {
                $scope.Legislation.DepartmentName = $scope.departmentList;
                $scope.Legislation.Signatory = $scope.signList;
                $http({
                    method: 'POST',
                    url: 'api/legislation/save',
                    data: $scope.Legislation
                }).then(function successCallback(response) {
                    $scope.legislationData = response.data;
					...
					$scope.Link = {
                        ...
                        LinkId: '',
                        LinkedId: $scope.linkedLegislationId,
                        LinkerId: $scope.legislationData.LegislationId,
                        LinkedCategoryCode: 'Y',
                        LinkerCategoryCode: 'Y',
                        LinkPath: $scope.Legislation.FilePath,
                        LinkedSituation: $scope.selectedRelation,
                        LinkerAnnouncementName: $scope.Legislation.LegislationName
                    }

                    //////////////
                    $scope.clear();
                    alert(response.data.LegislationId + " No'lu Yönetmelik " + "Başarıyla Eklendi !");
                    if ($scope.linkedLegislationId > 0) {
                        $scope.linksave();
                    }
                    location.reload();
    ...
    $scope.diffcompare = function () {
        ...
        var text1 = $scope.linkedLegislationText;
        var text2 = $scope.Legislation.Text;

        $http({
            method: 'POST',
            url: 'api/legislation/diff',
    ...
    $scope.getLinkedText = function (id1) {
        $http.get('api/legislation/getlegislationtext/' + id1).success(function (data) {
            $scope.comparedtext = "";
	...
	$scope.getLinkerText = function (id2) {
        $http.get('api/legislation/getlegislationtext/' + id2).success(function (data) {
    ...
    $scope.diffcompareview = function (combination) {
        ...
        var idArr = str.split("~");
        if (idArr[1] == null) {
            alert("Yönetmelik Bağlı Değil İçerik Karşılaştırılamadı !");
        }
        else {
	...
	$scope.calculateDiff = function () {
        ...
        $http({
            method: 'POST',
            url: 'api/legislation/diffcompare',
            contentType: "application/json",
    ...
    $scope.remove = function (LegislationId) {
        $scope.loading = true;
        //var Id = this.Announcement.AnnouncementId;
        $http({
            method: 'PUT',
            url: 'api/legislation/remove/' + LegislationId,
            data: { data: LegislationId }
        }).then(function successCallback(response) {
            //$scope.announcementData.push(response.data);

            alert("Yönetmelik Başarıyla Silindi !");
    ...
    $scope.uploadFiles = function () {
        var request = {
            method: 'POST',
            url: 'api/legislation/fileupload',
            data: formdata,
        ...
        $http(request)
            .success(function (d) {
                ...
                $scope.Legislation.FilePath = d.FileName;
    ...
    $scope.downloadFile = function (name, LegislationId) {
        $http({
            method: 'GET',
            url: 'api/legislation/download' + LegislationId,
            params: {
                name: name,
                AnnId: AnnId
            },
