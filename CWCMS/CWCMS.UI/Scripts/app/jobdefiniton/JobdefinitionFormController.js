app.controller("jobdefinitionFormController", ["$scope", "$http", "$location", '$timeout', '$window', function ($scope, $http, $location, $timeout, $window) {


	$scope.jobdefinitionData = '';
	$scope.Jobdefinition = {
        JobdefinitionId: '',
        JobdefinitionName: '',
		Text: '',
    ...
    $scope.modalclear = function () {
        $scope.Jobdefinition.JobdefinitionName = '';
        $scope.Jobdefinition.Text = '';
        $scope.Jobdefinition.DepartmentCode= '';
        $scope.Jobdefinition.SignId='';
    ...
    $scope.diffanycompareclear = function () {
        $scope.comparedtext = "";
        $scope.jobdefinition.LinkerId = "";
        $scope.jobdefinition.JobdefinitionId = "";
    ...
    $scope.countAnnouncements = function () {
        if ($scope.listingMode == 1) {
            $http.get('api/jobdefinition/getcount').success(function (data) {
        ...
        .error(function () {
            ...
            $http.get('api/jobdefinition/HeaderGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
		.error(function () {
            ...
            $http.get('api/jobdefinition/ContentGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
        .error(function () {
    ...
    $scope.getJsonFromUrl = function () {
        ...
        $http.get('api/jobdefinition/search/' + $scope.stringnumcomb).success(function (data) {

            $scope.jobdefinitions = data;
    ...
    $scope.alljobdefinitiondata = function () {
        $scope.stringnumcomb = $scope.firstIndex + "-" + $scope.itemsperpage;
        $http.get('api/jobdefinition/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.CircularList = function () {
        $http.get('api/jobdefinition/Outlist').success(function (data) {
            $scope.Circular = data;
            $scope.loading = false;
        }).error(function () {
            $scope.error = "An Error has occured while loading posts!"
    ...
    $scope.signList = function () {
        $http.get('api/jobdefinition/takeSign').success(function (data) {
            $scope.signList = data;
            $scope.SignList = data;
            $scope.loading = false;
    ...
    $scope.refreshSignList = function () {
        $http.get('api/jobdefinition/takeSign')
    ...
    $scope.departmentList = function () {
        $http.get('api/jobdefinition/takeDepartment').success(function (data) {
    ...
    $scope.refreshDepartment = function () {
        $http.get('api/jobdefinition/takeDepartment')
              .success(function (data) {
                  $scope.Department = data;
              });
    }
    ...
    $scope.listrefresh = function () {
        ...
        if (typeof pageLinkSize[2] == 'undefined' || typeof pageLinkSize[4] == 'undefined' || typeof pageLinkSize[6] == 'undefined') {
            ...
            if (pageLinkSize[6] == '1') {
                ...
                $scope.jobdefinitions = []; $http.get('api/jobdefinition/searchheader/' + $scope.query1).success(function (data) {
                    $scope.jobdefinitions = data;
                ...
                $scope.jobdefinitions = []; $http.get('api/jobdefinition/searchcontent/' + $scope.query2).success(function (data) {
                    $scope.jobdefinitions = data;
                    $scope.total_count = data.total_count;
    ...
    $scope.addtothelink = function (JobdefinitionId) {

        $http.get('api/jobdefinition/getjobdefinitionname/' + JobdefinitionId).success(function (data) {
            $scope.linkedJobdefinitionName = data.JobdefinitionName;
            $scope.linkedJobdefinitionId = data.JobdefinitionId;
            $scope.linkedJobdefinitionText = data.Text;
	...
	// Viewing Circular
    $scope.viewJobdefinition = function (JobdefinitionId) {
        $http.get('api/jobdefinition/getjobdefinitionname/' + JobdefinitionId).success(function (data) {
            $scope.viewedJobdefinition = data;
            $scope.loading = false;
        })
    }
    ///
    ...
    $scope.relateTypes = [
        { types: 'Görev Tanımının Kapsamı Genişletildi' },
        { types: 'Görev Tanımı Geçerliliğini Yitirdi' },
	...
	$scope.clear = function () {
        ...
        $scope.Jobdefinition.JobdefinitionName = '';
        $scope.Jobdefinition.Text = '';
        $scope.Jobdefinition.FilePath = '';
        $scope.Jobdefinition.DepartmentCode = '';
    }
    $scope.refresh = function () {
        $http.get('api/jobdefinition/search')
              .success(function (data) {
                  $scope.Jobdefinition = data;
              });
    }
	
	$scope.dateFix = function () {
        var date = $scope.Jobdefinition.DateExpireTime;
	...
	$scope.save = function () {
        {
            if ($scope.Jobdefinition.JobdefinitionName == "" || $scope.Jobdefinition.Text == "") {
                alert("Lütfen Görev Tanımı Bölümünü Boş Bırakmayınız.");
            } else {
                $scope.Jobdefinition.DepartmentName = $scope.departmentList;
                $scope.Jobdefinition.Signatory = $scope.signList;
                $http({
                    method: 'POST',
                    url: 'api/jobdefinition/save',
                    data: $scope.Jobdefinition
                }).then(function successCallback(response) {
                    $scope.jobdefinitionData = response.data;
					...
					$scope.Link = {
                        ...
                        LinkId: '',
                        LinkedId: $scope.linkedJobdefinitionId,
                        LinkerId: $scope.jobdefinitionData.JobdefinitionId,
                        LinkedCategoryCode: 'G',
                        LinkerCategoryCode: 'G',
                        LinkPath: $scope.Jobdefinition.FilePath,
                        LinkedSituation: $scope.selectedRelation,
                        LinkerAnnouncementName: $scope.Jobdefinition.JobdefinitionName
                    }

                    //////////////
                    $scope.clear();
                    alert(response.data.JobdefinitionId + " No'lu Görev Tanımı " + "Başarıyla Eklendi !");
                    if ($scope.linkedJobdefinitionId > 0) {
                        $scope.linksave();
                    }
                    location.reload();
	...
	$scope.linksave = function () {
        {
            $http({
                method: 'POST',
                url: 'api/jobdefinition/LinkSave',
                data: $scope.Link
            }).then(function successCallback(response) {
    ...
    $scope.diffcompare = function () {
        ...
        var text1 = $scope.linkedJobdefinitionText;
        var text2 = $scope.Jobdefinition.Text;

        $http({
            method: 'POST',
            url: 'api/jobdefinition/diff',
    ...
    $scope.getLinkedText = function (id1) {
        $http.get('api/jobdefinition/getjobdefinitiontext/' + id1).success(function (data) {
            $scope.comparedtext = "";
	...
	$scope.getLinkerText = function (id2) {
        $http.get('api/jobdefinition/getjobdefinitiontext/' + id2).success(function (data) {
    ...
    $scope.diffcompareview = function (combination) {
        ...
        var idArr = str.split("~");
        if (idArr[1] == null) {
            alert("Görev Tanımı Bağlı Değil İçerik Karşılaştırılamadı !");
        }
        else {
	...
	$scope.calculateDiff = function () {
        ...
        $http({
            method: 'POST',
            url: 'api/jobdefinition/diffcompare',
            contentType: "application/json",
    ...
    $scope.remove = function (JobdefinitionId) {
        $scope.loading = true;
        //var Id = this.Announcement.AnnouncementId;
        $http({
            method: 'PUT',
            url: 'api/jobdefinition/remove/' + JobdefinitionId,
            data: { data: JobdefinitionId }
        }).then(function successCallback(response) {
            //$scope.announcementData.push(response.data);

            alert("Görev Tanımı Başarıyla Silindi !");
    ...
    $scope.uploadFiles = function () {
        var request = {
            method: 'POST',
            url: 'api/jobdefinition/fileupload',
            data: formdata,
        ...
        $http(request)
            .success(function (d) {
                ...
                $scope.Jobdefinition.FilePath = d.FileName;
    ...
    $scope.downloadFile = function (name, JobdefinitionId) {
        $http({
            method: 'GET',
            url: 'api/jobdefinition/download' + JobdefinitionId,
            params: {
                name: name,
                AnnId: AnnId
            },
