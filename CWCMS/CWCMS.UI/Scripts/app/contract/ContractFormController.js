app.controller("contractFormController", ["$scope", "$http", "$location", '$timeout', '$window', function ($scope, $http, $location, $timeout, $window) {
    ...
    $scope.modalclear = function () {
        ...
        $scope.Contract.Text = '';
        $scope.Contract.DepartmentCode= '';
        $scope.Contract.SignId='';
        $scope.linkedContract = '';
        $scope.matchedtext ='';
    ...
    $scope.countAnnouncements = function () {
        if ($scope.listingMode == 1) {
            $http.get('api/contract/getcount').success(function (data) {
        ...
        .error(function () {
            ...
            $http.get('api/contract/HeaderGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
		.error(function () {
            ...
            $http.get('api/contract/ContentGetCount/' + $scope.search).success(function (data) {
                $scope.totalData = data;
                $scope.loading = false;
            })
        .error(function () {
    ...
    $scope.getJsonFromUrl = function () {
        ...
        $http.get('api/contract/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.allcontractdata = function () {
        $scope.stringnumcomb = $scope.firstIndex + "-" + $scope.itemsperpage;
        $http.get('api/contract/search/' + $scope.stringnumcomb).success(function (data) {
    ...
    $scope.CircularList = function () {
        $http.get('api/contract/Outlist').success(function (data) {
            $scope.Circular = data;
            $scope.loading = false;
        }).error(function () {
            $scope.error = "An Error has occured while loading posts!"
    ...
    $scope.signList = function () {
        $http.get('api/contract/takeSign').success(function (data) {
            $scope.signList = data;
            $scope.SignList = data;
            $scope.loading = false;
    ...
    $scope.refreshSignList = function () {
        $http.get('api/contract/takeSign')
    ...
    $scope.departmentList = function () {
        $http.get('api/contract/takeDepartment').success(function (data) {
    ...
    $scope.refreshDepartment = function () {
        $http.get('api/contract/takeDepartment')
              .success(function (data) {
                  $scope.Department = data;
              });
    }
    ...
    $scope.listrefresh = function () {
        ...
        if (typeof pageLinkSize[2] == 'undefined' || typeof pageLinkSize[4] == 'undefined' || typeof pageLinkSize[6] == 'undefined') {
            ...
            if (pageLinkSize[6] == '1') {
                ...
                $scope.contracts = []; $http.get('api/contract/searchheader/' + $scope.query1).success(function (data) {
                    $scope.contracts = data;
                ...
                $scope.contracts = []; $http.get('api/contract/searchcontent/' + $scope.query2).success(function (data) {
                    $scope.contracts = data;
                    $scope.total_count = data.total_count;
    ...
    $scope.addtothelink = function (ContractId) {

        $http.get('api/contract/getcontractname/' + ContractId).success(function (data) {
            $scope.linkedContractName = data.ContractName;
            $scope.linkedContractId = data.ContractId;
            $scope.linkedContractText = data.Text;
    ...
    $scope.relateTypes = [
        { types: 'Sözleşmenin Kapsamı Genişletildi' },
        { types: 'Sözleşme Geçerliliğini Yitirdi' },
	...
	$scope.clear = function () {
        ...
        $scope.Contract.ContractName = '';
        $scope.Contract.Text = '';
        $scope.Contract.FilePath = '';
        $scope.Contract.DepartmentCode = '';
    }
    $scope.refresh = function () {
        $http.get('api/contract/search')
              .success(function (data) {
                  $scope.Contract = data;
              });
    }
	...
	$scope.dateFix = function () {
        var date = $scope.Contract.DateExpireTime;
	...
	$scope.save = function () {
        {
            if ($scope.Contract.ContractName == "" || $scope.Contract.Text == "") {
                alert("Lütfen Sözleşme Bölümünü Boş Bırakmayınız.");
            } else {
                $scope.Contract.DepartmentName = $scope.departmentList;
                $scope.Contract.Signatory = $scope.signList;
                $http({
                    method: 'POST',
					...
					$scope.Link = {
                        ...
                        LinkId: '',
                        LinkedId: $scope.linkedContractId,
                        LinkerId: $scope.contractData.ContractId,
                        LinkedCategoryCode: 'S',
                        LinkerCategoryCode: 'S',
                        LinkPath: $scope.Contract.FilePath,
                        LinkedSituation: $scope.selectedRelation,
                        LinkerAnnouncementName: $scope.Contract.ContractName
                    }

                    //////////////
                    $scope.clear();
                    alert(response.data.ContractId + " No'lu Sözleşme " + "Başarıyla Eklendi !");
                    if ($scope.linkedContractId > 0) {
                        $scope.linksave();
                    }
                    location.reload();
    ...
    $scope.diffcompare = function () {
        ...
        var text1 = $scope.linkedContractText;
        var text2 = $scope.Contract.Text;

        $http({
            method: 'POST',
            url: 'api/contract/diff',
    ...
    $scope.getLinkedText = function (id1) {
        $http.get('api/contract/getcontracttext/' + id1).success(function (data) {
            $scope.comparedtext = "";
	...
	$scope.getLinkerText = function (id2) {
        $http.get('api/contract/getcontracttext/' + id2).success(function (data) {
    ...
    $scope.diffcompareview = function (combination) {
        ...
        var idArr = str.split("~");
        if (idArr[1] == null) {
            alert("Sözleşme Bağlı Değil İçerik Karşılaştırılamadı !");
        }
        else {
    ...
    $scope.remove = function (ContractId) {
        $scope.loading = true;
        //var Id = this.Announcement.AnnouncementId;
        $http({
            method: 'PUT',
            url: 'api/contract/remove/' + ContractId,
    ...
    $scope.uploadFiles = function () {
        ...
        $http(request)
            .success(function (d) {
                ...
                $scope.Contract.FilePath = d.FileName;
    ...
    $scope.downloadFile = function (name, ContractId) {
        $http({
            method: 'GET',
            url: 'api/contract/download' + ContractId,
            params: {
                name: name,
                AnnId: AnnId
            },
